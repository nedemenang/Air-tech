...
class TestRoleEndpoints(BaseTestCase):
    ...
    def test_get_role_list_endpoint(self):

        response = self.client().get(self.make_url('/role/'), headers=self.headers())
    ...
    def test_get_single_role_endpoint(self):
    
        response = self.client().get(self.make_url('/role/1'), headers=self.headers())
